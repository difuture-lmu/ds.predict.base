...
It is necessary to register the assign and aggregate methods in the OPAL administration. These methods are registered automatically when publishing the package on OPAL (see [`DESCRIPTION`](https://github.com/difuture/ds.predict.base/blob/master/DESCRIPTION)).
...
```{r, echo=FALSE}
knitr::kable(installed.packages()[client_pkgs, "Version", drop = FALSE])
```
